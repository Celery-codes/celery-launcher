...
// Uses inst.folderName if set (new instances), falls back to inst.id
function getInstanceDir(instanceId) {
  const Store = require('electron-store');
  const inst  = new Store().get('instances',[]).find(i => i.id === instanceId);
  const name  = inst?.folderName || instanceId;
  const dir   = path.join(global.paths.INSTANCES_DIR, name);
  if (!fs.existsSync(dir) && inst?.folderName) {
    const fb = path.join(global.paths.INSTANCES_DIR, instanceId);
...
async function installMod(instanceId, mod, source, onProgress) {
  ...
  if (source==='modrinth') {
    ...
    if (!mod.downloadUrl) throw new Error('No download URL');
  ...
  const metaFile= path.join(metaDir,'mods.json');
  if (idx>=0) {
    meta[idx]={ id:mod.id, slug:mod.slug, title:mod.title, filename, source,
      enabled:true, installedAt:new Date().toISOString(), iconUrl:mod.iconUrl, projectId:mod.id };
    fs.writeFileSync(metaFile, JSON.stringify(meta,null,2));
  }
  updateInstanceModCount(instanceId, meta.length);
  onProgress({status:'done',message:'Installed!',percent:100});
  return { deps:{ required:[], optional:[] } };
}
  ...
  for (let i=0;i<meta.length;i++) {
    ...
    try {
      if (mod.source==='modrinth') {
        ...
        const modsDir=path.join(instanceDir,'mods');
        [path.join(modsDir,mod.filename),path.join(modsDir,mod.filename+'.disabled')].forEach(p=>{if(fs.existsSync(p))fs.unlinkSync(p);});
        onProgress({message:`Updating ${mod.title}...`,current:i+1,total:meta.length});
        await downloadFile(pf.url, path.join(modsDir, mod.enabled===false?pf.filename+'.disabled':pf.filename));
...
async function checkMissingDependencies(instanceId) {
  ...
  for (const mod of meta) {
    if (!mod.enabled||!mod.dependencies?.length) continue;
