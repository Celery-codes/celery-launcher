...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  window.launcher.onLaunchStatus((data) => {
    ...
    if (data.status === 'running') { launchRunning = true; launchStartTime = Date.now(); }
...
function renderInstanceGrid(list, search = '', loaderFilter = 'all') {
  ...
  grid.innerHTML = filtered.map(inst => {
    const playtime = formatPlaytime(inst.playtimeSeconds);
    const lastPlayed = inst.lastPlayed ? new Date(inst.lastPlayed).toLocaleDateString() : null;
    return `
    <div class="icard ${inst.id === selectedInstanceId ? 'sel' : ''}" onclick="selectInstance('${inst.id}')" id="icard-${inst.id}">
      ...
      <div class="imeta">${inst.mcVersion} &middot; ${inst.loader}${inst.mods > 0 ? ' &middot; ' + inst.mods + ' mods' : ''}</div>
      ${playtime ? `<div class="imeta" style="color:var(--text3);margin-top:2px;">&#9201; ${playtime}${lastPlayed ? ' &middot; ' + lastPlayed : ''}</div>` : ''}
...
function toggleCtxMenu(id) {
  ...
  if (openMenuId && openMenuId !== id) {
    const old = document.getElementById('ctx-' + openMenuId);
...
function toggleSnapshots() {
  ...
  const snaps = show ? mcVersionData.snapshots.map(v => `<option value="${v.id}">${v.id} (snapshot)</option>`).join('') : '';
  sel.innerHTML = mcVersionData.releases.map((v, i) => `<option value="${v.id}">${v.id}${i === 0 ? ' (latest)' : ''}</option>`).join('') + snaps;
}
...
async function onLoaderChange() {
  ...
  label.textContent = loader + ' Loader Version';
  if (loader === 'Fabric') {
    if (!fabricData) fabricData = await window.launcher.getFabricVersions();
    populateLoaderVersionSelect('newLoaderVer', fabricData.loaders);
...
async function createInstance() {
  ...
  const instance = {
    ...
    playtimeSeconds: 0, lastPlayed: null,
  ...
  if (loader !== 'Vanilla' && loader !== 'Forge' && loader !== 'NeoForge') {
    window.launcher.onLoaderApiProgress((p) => {
      document.getElementById('launchStatus').textContent = p.message || '';
    });
...
async function launchGame() {
  ...
  if (!activeAccountUuid) {
    ...
    showPanel('accounts', document.getElementById('nav-accounts'));
    return;
  }
  launchRunning = true;
  setLaunchBtnState(true);
  const inst = instances.find(i => i.id === selectedInstanceId);
