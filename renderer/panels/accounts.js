  ...
  if (!accounts.length) {
    ...
    list.innerHTML = accounts.map(acc => `
      <div class="acc-row ${acc.uuid === activeAccountUuid ? 'active' : ''}">
        <div class="acc-av" style="width:36px;height:36px;border-radius:8px;font-size:13px;" id="av-${acc.uuid}">${acc.username.substring(0,2).toUpperCase()}</div>
        <div style="flex:1;">
          <div style="font-size:13px;font-weight:500;color:var(--text);">${escHtml(acc.username)}</div>
          <div style="font-size:11px;color:var(--text3);">Microsoft · Java Edition${acc.ownsGame===false?' · ⚠ No license':''}</div>
        </div>
    ...
    for (const acc of accounts) loadAccountSkin(acc);
  }
}
...
async function addAccount() {
  ...
  status.textContent = 'A Microsoft sign-in window will open. Choose your account to continue.';
...
async function loadAccountSkin(account) {
  ...
  try {
    ...
    if (result.success && result.dataUrl) {
      ...
      if (av && account.uuid === activeAccountUuid) {
      ...
      const rowAv = document.getElementById('av-' + account.uuid);
      if (rowAv) {
        rowAv.innerHTML = `<img src="${result.dataUrl}" style="width:36px;height:36px;border-radius:8px;image-rendering:pixelated;" alt="${account.username}">`;
      }
